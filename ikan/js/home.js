    ...
    'History': {
        ...
        'Init':function(){
            ...
            $('.history').click(function(e){
				...
				$("#closeh,.mac_user").click(function(){
			...
			html += '<div class="mac_history_box">';
            html +='<ul class="vodlist">'
            if(jsondata.length > 0){
				html +='<li class="clear_btn"><a target="_self" href="javascript:void(0)" onclick="MAC.History.Clear();">清空</a></li>';
                for($i=0; $i<jsondata.length; $i++){
                    html +='<li class="vodlist_item">';
                    html +='<a href="'+jsondata[$i].link+'" title="'+jsondata[$i].name+'"><div class="vodlist_thumb" style="background-image:url(http://images.100tb-cdn.com'+jsondata[$i].pic+')"></div><div class="vodlist_titbox"><p class="vodlist_title">'+jsondata[$i].name+'</p></div></a></li>';
                }
            }else{
                html +='<li class="history_no"><i class="iconfont"></i>暂无观看记录<p class="user_log_tips"><a class="mac_user" href="javascript:void(0)">登录查看更多</a></p></li>';
            }
			html +='</ul>'
			html += '</div>';
            html += '</div>';
        ...
        'Clear': function(){
            ...
            $('.mac_history_box').html('<div class="history_clear">已清空观看记录</div>');
        },
    },
...
$(function(){
    ...
    MAC.Timming();
							...
							$(function(){
								let _video = $("<video class='video_preview' muted autoplay='autoplay' loop='loop'></video>");
								$("a.vodlist_thumb").hover(function(){
									//$(this).css("backgroundImage","none");
									var _src = $(this).data("preview");
									_video.attr("src",_src);
									$(this).append(_video);
								},function(){
									//$(this).css("backgroundImage","url('"+$(this).data("original")+"')");
									_video.remove();
								});
							});
